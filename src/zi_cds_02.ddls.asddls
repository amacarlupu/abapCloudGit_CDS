  ...
  as select from /dmo/customer
{
  key customer_id as CustomerID,
      ...
      //      También se puede agregar validaciones para distintos campos
      case
          when country_code = 'US' or country_code = 'ES' then case title
                                                               when 'Mr.' then 'US/ES - Mr.'
                                                               when 'Mrs.' then 'US/ES - Mrs.'
                                                               else 'US/ES - Different Tile'
                                                           end
          when title = 'Mr.' then last_name
          when cast( '20260222' as abap.dats ) < dats_add_days( $session.system_date, -30, 'NULL') then 'Lower Data'
          else 'No condiiton applied'
      end         as Case2
}
